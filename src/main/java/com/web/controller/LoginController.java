...
*/
    ...
    public String loginComplete(HttpSession session) {
        /*
        SecurityContextHolder에서 인증된 정보를 OAuth2Authentication 형태로 받아온다.
        OAuth2Authentication은 기본적인 인증에 대한 정보뿐만 아니라,
        OAuth2 인증과 관련된 정보도 함께 제공한다.
         */
        OAuth2Authentication authentication = (OAuth2Authentication)
                SecurityContextHolder.getContext().getAuthentication();
                /*
                리소스 서버에서 받아온 개인 정보를 getDetails()를 사용해 Map 타입으로 받을 수 있다.
                 */
        @SuppressWarnings("unchecked")
        Map<String, String> map = (HashMap <String, String>)
                authentication.getUserAuthentication().getDetails();
                /*
                세션에 빌더를 사용하여 인증된 User 정보를 User 객체로 변환하여 저장한다.
                */
